...
st.set_page_config(page_title="中国西北沙尘短临预测", layout="wide")
st.title("中国西北沙尘暴风速短时临近预测系统")
...
if page == "首页":
    st.markdown(
        """
本系统定位于**中国西北区域级 0-6 小时短时临近预测**（河西走廊-宁夏北部-陕北西段），输出：
- 1h / 3h / 6h 风速预测
- 沙尘风险等级（0/1/2/3）
- 达到预警阈值概率
- 解释性结果（时间注意力、站点图注意力、特征重要性）
        """
    )

elif page == "数据说明":
    st.subheader("中国西北版本数据源")
    st.write("1. NOAA NCEI Global Hourly 中国站点观测（优先）")
    st.write("2. Open-Meteo 中国西北背景场（回退与补强）")
    st.write("3. Open-Meteo Elevation + 沙源距离 proxy 静态因子")
    m = Path("data/raw/download_manifest.json")
    if m.exists():
        st.json(json.loads(m.read_text(encoding="utf-8")))

elif page == "训练结果":
    st.subheader("主模型指标（中国西北）")
    ...
    if bp.exists():
        st.subheader("Baselines 指标")
        st.json(json.loads(bp.read_text(encoding="utf-8")))

elif page == "单点预测":
    st.subheader("单点预测（中国西北站点）")
    stations_resp = requests.get(f"{API_URL}/stations", timeout=20).json()
    station_items = stations_resp["stations"]
    station_map = {f"{s['station_name']} ({s['station_id']})": s["station_id"] for s in station_items}
    options = list(station_map.keys())
    station_key = st.selectbox("站点", options, index=0)
    station_id = station_map[station_key]
    wind_override = st.number_input("可选: 当前风速覆盖值 (m/s)", min_value=0.0, value=6.0, step=0.5)
